...
function hookLoader(matcherOrRoot, opts) {
    ...
    instrumenter =  Instrumenter.createInstrumenter();
...
function mergeClientCoverage(obj) {
    ...
    fs.writeFileSync(__dirname +"/.nyc_output/out.json", JSON.stringify(coverage));

    cp.spawn('nyc report --reporter=html', null, {shell: true});
}
